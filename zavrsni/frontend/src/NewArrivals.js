...
const NewArrivals = () => {
    ...
    async function getProducts (){
        ...
        const productList = await fetch(`http://localhost:3001/product/newProducts`, options);
                        ...
                        return(
                            <div className="col-3 p-4">
                                ...
                                <div className="text-dark text-center">
                                    <b>{products[it].name}</b><br/>
                                    
                                    <b>Date: </b><span style={{color:'red'}}>{(new Date(products[it].date)).toLocaleString()}</span>
                                    
                                </div>
                                { products[it].on_stock > 0 ? shades.map((shade, index) => {
                                            if ((shade.product_id === products[it]._id) && (shade.quantities > 0)){
                                                return(
                                                    <div className='shadesDiv3'>
                                                        <div className="shade3" style={{background:shade.hex}} onClick={() => { addToCart(products[it], shade)}}></div>
                                                    </div>
                                                )
                                            }
                                }):(<p></p>)}
